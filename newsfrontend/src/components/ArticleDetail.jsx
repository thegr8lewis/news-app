...
const ArticleDetail = ({ language, onLanguageChange }) => {
      ...
      try {
        ...
        if (translationId) {
          ...
          if (translatedArticle.language === language) {
            navigate(`/articles/${translationId}`);
            return;
          }
        }
        ...
        const allArticles = await getArticles();
        const translation = allArticles.find(a => 
          (a.translation_id === article.id && a.language === language) ||
          (a.slug === article.slug && a.language === language)
        );

        if (translation) {
          navigate(`/articles/${translation.id}`);
        }
      } catch (err) {
  ...
  }, [language, article, navigate, translationId]);
