...
# Translation Analysis Model Setup (optional ML dependencies)
MODEL = None
try:
    ...
    logger.info("ML dependencies not available - using basic analysis")
...
def analyze_text_semantics(original, translated):
    ...
    try:
        ...
        try:
            ...
            overall_score = sum(scores[:min_chunks]) / len(scores[:min_chunks])
...
def analyze_translation(request):
    ...
    try:
        ...
        # Use advanced semantic analysis if model is available
        if MODEL:
            analysis_result = analyze_text_semantics(original_text, translated_text)
            if analysis_result:
                analysis = generate_analysis_report(analysis_result, language)
                if analysis:
                    return Response(analysis)
        
        # Fallback to simple analysis if semantic analysis fails
        analysis = {
